...
#GURU new changes for django 1.4.1
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for u in users:
                ...
                messages.add_message(request, messages.SUCCESS, message=_("An email has been sent with your temporary login key"))
...
def send_validation_email(request):
    if not request.user.is_authenticated():
        ...
        messages.add_message(request, messages.SUCCESS, message=_("A message with an email validation link was just sent to your address."))
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if is_new_pass:
                ...
                messages.add_message(request,messages.SUCCESS, message=_("New password set"))
                if not request.user.is_superuser:
                    form = ChangePasswordForm(user=user_)
            else:
                messages.add_message(request,messages.SUCCESS, message=_("Your password was changed"))
...
def remove_external_provider(request, id):
    ...
    messages.add_message(request, messages.SUCCESS, message=_("You removed the association with %s") % association.provider)
    association.delete()
...
def login_and_forward(request, user, forward=None, message=None):
    ...
    if pending_data and (user.email_isvalid or pending_data['type'] not in settings.REQUIRE_EMAIL_VALIDATION_TO):
        ...
        if submission_time < datetime.datetime.now() - datetime.timedelta(minutes=int(settings.HOLD_PENDING_POSTS_MINUTES)):
            ...
            messages.add_message(request, messages.SUCCESS, message=(_("You have a %s pending submission.") % pending_data['data_name']) + " %s, %s, %s" % (
